...
<title>User | Kami Digital Printing Surabaya</title>
...
<main class="p-3">
                ...
                <div class="relative rounded-lg shadow bg-gray-700">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        ...
                        <form class="space-y-6" action="" method="post" enctype="multipart/form-data">
                            ...
                            <div>
                                <label for="photo_user" class="block mb-2 text-sm font-medium text-white">Foto User</label>
                                <input id="file_user" type="file" name="photo_user" class="border border-gray-500 text-white w-full rounded-lg" placeholder="">
                            </div>
                            <div>
                                <label for="email_user" class="block mb-2 text-sm font-medium text-white">Email</label>
                                <input type="email" id="email_user" name="email_user" class="border text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 bg-gray-600 border-gray-500 placeholder-gray-400 text-white" placeholder="">
                            </div>
                            <div>
    ...
    <!-- Error Message -->
    <?php if (!empty($errPilihanGambar) || !empty($errEkstensiGambar) || !empty($errUkuranGambar) || !empty($errUser) || !empty($errNamaBaru)) : ?>
        <div class="flex items-center p-4 mb-4 text-sm rounded-lg text-zinc-800 bg-red-400" role="alert">
            ...
            <div>
                ...
                <?php if (!empty($errPilihanGambar)) : ?>
                    <span class="font-bold"><?= $errPilihanGambar ?></span>
                <?php elseif (!empty($errEkstensiGambar)) : ?>
                    <span class="font-bold"><?= $errEkstensiGambar ?></span>
                <?php elseif (!empty($errUkuranGambar)) : ?>
                    <span class="font-bold"><?= $errUkuranGambar ?></span>
                <?php elseif (!empty($errUser)) : ?>
                    <span class="font-bold"><?= $errUser ?></span>
                <?php elseif (!empty($errNamaBaru)) : ?>
                    <span class="font-bold"><?= $errNamaBaru ?></span>
                <?php endif; ?>
            </div>
        </div>
    <?php endif; ?>
    ...
    <section class="pb-6">
                    ...
                    <?php foreach ($dataUser as $rowUser) : ?>
                        <tr class="border-b border-zinc-700">
                            ...
                            <td class="py-2 px-4 text-center border-r border-zinc-700">
                                <img src="../../../app/assets/users/user-photo/<?= $rowUser['photo_user'] ?>" onerror="this.src='../../../app/assets/users/user-photo/error/user.png';" alt="" class="w-20 mx-auto cursor-pointer" data-modal-target="photo-modal" data-modal-toggle="photo-modal">
                            </td>
        ...
        <div id="photo-modal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative w-full max-w-md max-h-full">
                <!-- Modal content -->
                <div class="relative rounded-lg shadow bg-gray-700">
                    <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center hover:bg-gray-600 hover:text-white" data-modal-hide="photo-modal">
                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                        </svg>
                        <span class="sr-only">Close modal</span>
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        <h3 class="mb-4 text-xl font-medium text-white">Your Name Product</h3>
                        <img src="../client/user-photo/blabla.png" alt="" class="">
                    </div>
                </div>
            </div>
        </div>
